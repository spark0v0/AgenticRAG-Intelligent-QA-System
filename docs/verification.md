# 实际验证记录

日期：2026-09-18（Asia/Shanghai）。环境：Windows / PowerShell、Node 24.19、npm 11.6、Python 3.13、系统 Chrome。下列结论来自本次本机执行，不代表 CI 云端或所有供应方均通过。

## 最新增量：前端体验与组件整理

2026-09-18 在 `E:\AgenticRAG-Intelligent-QA-System` 的 `efaa631` 基础上完成。原目录仅作参考，未清理或改写原数据；本轮没有新增依赖、后端接口或数据库迁移。以下为本轮实际检查，下一节保留上轮验证记录。

| 实际执行 | 结果与范围 |
| --- | --- |
| ESLint / Prettier / vue-tsc / Vite | 通过；构建命令含类型检查 |
| Vitest | 9 项通过；沿用 SSE 分块、断流、会话隔离、草稿替换、安全 Markdown 与取消测试，新增明确引用/代码与链接排除、422 字段路径及敏感消息过滤 |
| Playwright | 3 条核心流程通过；问答停止/重试/恢复、运行页窄屏筛选与返回；供应商字段校验、422 后保留输入、`ok:false` 无绿色成功、保存/刷新/删除；60 条长会话 |
| 正常生产服务 | `http://127.0.0.1:8005/chat`，FastAPI 提供最新构建与真实 API；桌面 1440×1000、窄屏 390×844，空会话另检查 320px，无页面横向溢出 |
| 真实历史回看 | 复用已有真实检索运行，回答引用和运行页引用均定位并聚焦证据；瀑布选择、起止详情、JSON 下载、回到会话、手机端返回通过 |
| 主题和表单 | 浅色/深色、桌面/手机编辑表单，Key 均为空；手机端保存按钮位于视口内，内容独立滚动。已目视检查截图 |
| 网络与控制台 | 17 张正常页面截图没有拦截响应，pageerror/console error 为空，关键 API 返回正常；未增加真实模型调用 |
| 隐私检查 | 待交付工作树文件未匹配已知本机 API Key；`.env`、供应商库、会话库、测试缓存和构建目录被 Git 忽略。Git 本机身份为 GitHub noreply；发布前再次检查暂存区，发布状态以 Git 历史为准 |

具体修复：连接检查不再忽略业务 `ok`；供应商错误不再误报成图片校验；表单支持字段定位、重试保留和提交互斥；原先按时间戳无法确认的耗时统一显示「未记录」。样式从 `main.scss` / `workspace.scss` 的覆盖关系收敛到主题和业务模块，供应商表单/模型行、运行瀑布/节点详情各自维护。

截图及实际布尔检查见 `screenshots/workspace-metrics.json`。其中 `newModelRequest` 为 false，`previousModelVerification` 仅归档上一轮供应方联调。连接失败及 422 场景通过测试进程中的隔离响应验证，没有更改真实供应商地址、密钥或记录。测试夹具不进入产品运行路径。

最新构建体积（十进制 kB）：入口 JS **142.11 / gzip 55.32**，入口 CSS **42.86 / gzip 8.87**；供应商 JS **117.75 / gzip 39.73**，运行分析 JS **13.56 / gzip 5.34**，Markdown JS **182.49 / gzip 73.56**。60 条长消息夹具：导航到消息可见 **332ms**，填写输入确认 **19ms**；包含浏览器自动化开销，仅代表本机一次测量，不是纯 Vue 耗时或改善百分比，没有据此引入虚拟列表。

本轮未验证：未再次调用付费模型或真实制造上游失败；供应方生成、视觉、取消计费边界仍按上轮证据说明。未改 Python，因此未重复运行 Python/迁移测试；既有结果在下文。未触发或核验本轮云端 CI，也没有负载测试。未用屏幕阅读器验收。

仍有边界：引用只支持后端当前 `[Sx]` 明确约定，映射不证明模型引用正确；旧记录无映射不补造。生成验证属于后端当前进程，重启可能显示未验证。手机返回保留页内筛选，跨页/刷新仅恢复 URL 的 run ID。已有同步工具取消、单进程本机运行等限制不变。

## 上轮增量：供应商管理与运行分析

本轮在已发布版本基础上改造，工作区为新仓库。未新增前端依赖；增加 Element Plus 表单、弹窗、开关等按需导入，重做炭黑导航/白色工作区/钴蓝主操作，新增 `/providers` 和 `/runs`。

| 实际执行 | 结果 |
| --- | --- |
| vue-tsc / ESLint / Prettier / Vite | 类型、Lint、格式与生产构建通过 |
| Vitest | 沿用的 7 项核心逻辑测试通过 |
| Python | 本轮执行 `test_workbench_smoke.py` + `test_provider_settings.py` 共 7 项；覆盖原流式/取消/迁移，新增凭据保护、不回传/不回显、留空保留/清除、跨站拒绝、旧连接检查失效、运行分页与详情 |
| Playwright | 3 条场景通过：问答取消重试/历史/运行分析；供应商表单保存刷新删除与浏览器不缓存 Key；长会话交互 |
| 真实供应商 | DeepSeek 模型列表接口成功；以新本机供应商配置完成一次短问答，模型档案为 managed ID，后端 success |
| 页面与导出 | 最新生产服务 8004，桌面 1440×1000；390×844 问答/供应商/运行页无横向溢出；深色主题、弹窗、历史来源回看、JSON 下载通过；脚本/控制台 error 为空 |

这次仅新增 **一次真实短问答**：「仅回复“连接正常”，不要添加其他内容。」浏览器发送到输入恢复为 **9005ms**，不是首 token 耗时，也不是 Vue 渲染耗时。后续截图只读取已有数据，没有重发付费请求。运行分析的检索证据来自前一版本保存的真实 DeepSeek 问答；通过 SQLite backup API 复制到新工作区，原数据库不修改，真实历史和供应商库不提交。

最新截图为 `screenshots/workspace-*.png`，指标在 `workspace-metrics.json`；`real-*` 为前一版本真实服务证据，旧样例截图仍只作归档。所有新截图均来自运行中的产品页面，没有拦截 API 或注入模拟响应。API Key 输入为空；来源标题对 Windows 路径仅显示文件名，导出仍可能保留原始来源路径。

最新构建（十进制 kB，资源拆分可能随构建改变）：入口 JS 139.95 / gzip 54.28；供应商页面 JS 112.45 / gzip 37.90；运行分析 JS 11.15 / gzip 4.49；Markdown 懒加载 JS 181.80 / gzip 73.26；共享选择器 JS 128.21 / gzip 44.45。没有声称性能提升百分比。

隐私复核：已提交文件未匹配到原本机已知 API Key；`.env`、会话数据库和新供应商库排除在 Git 之外。提交身份改用 GitHub noreply。该检查不是对未知敏感文字的通用识别，也不能撤回外界已获取的历史副本。

本轮未验证：其他供应商实际生成/视觉接口、Windows 以外的凭据运行环境、公网/多用户部署。供应商连接只证明模型列表端点可读；模型能力开关仍为配置声明。运行瀑布不保存各轮完整旧草稿，不提供任意两稿差异比较；不能把规则 Critic 称作事实准确率。云端 CI 结果另以 GitHub Actions 为准。

## 项目核实

| 类别 | 代码结论 |
| --- | --- |
| 已有可用基础 | Vue 3/Pinia/Router 工程、SSE 协议、SQLite 存储；已有 RAG 智能体与 Chroma 混合检索、工具注册、规则评审 |
| 本轮修复 | 移除演示产品路径/模拟生成/模型错误伪成功；真实模型状态、异步连接关闭、模型选择一致性、UI 布局与元数据回看 |
| 可直接接入 | 配置中的模型档案、工具列表、会话历史、运行事件、来源、原有 API 与 `/api` 别名 |
| 仍有缺口 | 知识库管理、工具配置写入、复杂图谱与 Dify 发布；模型供应商写入已在最新增量完成 |

## 前一版本核心检查

| 实际执行 | 结果与覆盖 |
| --- | --- |
| `npm run lint` | 通过，src ESLint |
| `npm run format:check` | 通过，src Prettier |
| `npm run build` | vue-tsc 类型检查与 Vite 生产构建通过 |
| `npm run smoke` | 7 项通过：中文 UTF-8 分块/多帧、缺终态与身份不匹配、安全 Markdown、跨会话与草稿替换、断流恢复、旧模式指针迁移、取消完成竞争 |
| Python 核心冒烟 | 4 项：JSON 幂等迁移、不破坏原文件；取消首轮并重试及最终元数据；实际 SDK + MockTransport 增量/关闭；退役 mock 提供方不得成功 |
| 原有 Python 相关用例 | 参数校验、工具超时、DeepSeek 图片拒绝、视觉附件链路，共 4 项；生成回答使用测试目录夹具，非视觉模型实测 |
| Playwright | 2 项通过：问答/取消/重试/恢复/来源/工具筛选/SPA 刷新/390px 窄屏；60 条长消息恢复、编辑及回到底部 |
| 正常启动与代理 | 8000 生产页面及 5173 `/api/status` 实际可访问；返回 live、3 个模型档案、6 个工具 |

Python 共执行上述 8 个不同用例，没有跑全量算法评估或付费批量测试。测试模型数据仅从 `tests/` 注入；浏览器服务使用系统临时 SQLite/Chroma，正常运行不导入测试模块。

过程中发现并修复：Element Plus 默认宽度覆盖导致停止按钮被遮挡；读取系统状态时不必要地初始化模型 HTTP 客户端；演示键残留；模型错误被当作正常答案；普通请求自动尝试备用模型。系统 pytest 临时目录权限异常时改用新的 `.cache/pytest-*` 专用目录，没有清理原目录。

## 前一版本真实服务联调

通过 `tests/serve_real_smoke.py` 读取现有模型配置，使用 `.cache/real-service-smoke/` 隔离 SQLite/Chroma；检索范围限定仓库 README 和 src，未使用用户私人文档作为截图证据。脚本不会被 CI 自动执行。

- **DeepSeek V4 Pro / deepseek-v4-pro**：一次真实检索问答完成。实际经过 Router → Retriever → Generator → Critic，3 轮检索、1 轮生成、6 个结构化来源。没有为了截图替换模型输出。
- 问题：“用三点概括本地资料中 AgenticRAG 的核心模块，标注来源，回答不超过150字。”
- 浏览器从发送到首次可见增量 **22627ms**，到输入重新可用 **23975ms**。包含网络、工具选择、检索、供应方等待、传输与浏览器交互，不是纯模型 TTFT 或纯 Vue 渲染耗时。
- 最终文本与 SQLite 恢复值一致；刷新历史恢复通过。回答内容仍需人工核实，不能从一次调用推断准确率。
- 第二次真实请求在增量出现后停止，后端保存 `cancelled`，输入恢复。未自动重试该请求，未验证供应方内部是否停止计算/计费。
- `mode=demo` 返回 422；真实页面没有演示切换。页面脚本错误和控制台 error 记录为空。

自动截图读取已关闭 SSE 响应体曾失败；随后通过已保存运行补取证据，没有重新发送第一轮问答。记录见 `screenshots/real-service-metrics.json`。

## 前一版本界面与性能

真实截图：

- [问答与执行时间线](screenshots/real-workbench-desktop.png)
- [来源与证据](screenshots/real-sources.png)
- [窄屏问答](screenshots/real-workbench-mobile.png)
- [欢迎界面](screenshots/real-welcome.png)
- [工具中心](screenshots/real-tools.png)
- [系统概览](screenshots/real-system.png)
- [真实取消](screenshots/real-cancelled.png)

检查桌面 1440×1000 与窄屏 390×844：页面无横向溢出，输入/停止按钮可操作；另在最新生产构建检查了 320px 宽度及主题切换，输入操作区仍在视口内、无页面脚本错误。代码和模型表允许各自横向滚动。截图由页面直接截取，已目视检查。旧 `workbench-*.png` 属于退役演示版本，保留但不用于当前验收。

最近生产构建（十进制 kB）：入口 JS 134.23 / gzip 52.22，Chat 懒加载 JS 313.83 / gzip 118.88，执行详情 6.08 / gzip 2.63；工具 2.87、系统 3.03。入口 CSS 22.47，Chat CSS 26.84。

长会话夹具：60 条消息，其中 30 条回答各 2445 字符。导航到全部消息可见 378ms，填写输入并确认 22ms；包含自动化调度，不作为纯渲染耗时或通用性能保证。文件在 `frontend/test-results/evidence/history-metrics.json`。夹具首增量 853ms 不能当作公网模型指标。没有基线对照或提升百分比，也没有因此引入虚拟列表。

## 未验证与限制

- Ollama、Xinference、豆包视觉、真实天气/网络搜索、完整 Dify 发布未进行本轮外部联调。图片链路已实现并以隔离夹具验证，不声称真实视觉识别已验收。
- 本次真实问答没有触发 Critic 再生成；前端多轮草稿替换由 Vitest 验证。更多真实修订和外部故障组合留作后续。
- GitHub Actions 已配置，未推送或远端运行。没有负载压测、商业部署或精度评价。
- 本机单 worker；尚无认证。Chroma 首次索引初始化同步执行，历史全量加载、图片 base64 存储适合当前规模，后续应分页和媒体文件化。
- OpenAI 兼容请求取消会关闭异步连接；同步工具和 Ollama/Xinference 在线程内运行，可能持续至超时。前端 AbortController 不能证明上游模型停止计算。
- JSON 是一次幂等导入，不做后续双向同步；旧记录缺失的来源/轨迹无法补造。原始数据和退役样例保留，没有 reset 或清库。
- 规则评审、HashEmbeddings、配置式图谱、示例 MCP 属于当前实现边界，能力审计表不能替代实际验证。

P2 缺口和最小接口建议见 `roadmap.md`。优先维护可靠问答与存储，再按实际需要扩展管理能力。

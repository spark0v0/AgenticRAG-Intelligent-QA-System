...
本轮补充可讲解的前端职责：检索范围用有标签的单选组表达，并以类型约束发送到后端；请求发送时快照保留范围，不被后续选择覆盖。回答展示实际检索范围、证据数量、后端首增量计时；执行详情解释检索停止原因与具体证据检查项。检索算法、规划与模型调用属于后端改造，不能全部包装成前端独立贡献。具体实现与真实验证范围见 `agenticrag-audit.md`。
...
见 [本轮知识库前端求职材料](knowledge-career.md)。该记录区分既有能力、本轮新增能力、真实本地嵌入与隔离回答夹具。

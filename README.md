...
5. 点击回答中有高亮的 `[S1]` 等引用，定位对应证据；只有后端 `source_map` 中存在的编号可点击，普通数字和未知引用保持原样。从回答下方打开执行详情，或进入「运行分析」按问题/状态查找记录。选择瀑布节点查看起止时间、输入输出、节点期间工具调用；切换证据/最终回答，导出单次运行 JSON，或回到原会话。手机端用「返回运行列表」继续筛选。刷新恢复已保存历史；旧 JSON 会话可能没有运行元数据。
